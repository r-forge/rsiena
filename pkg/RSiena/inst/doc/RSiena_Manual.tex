...
This version was renewed for \RS version 1.1-243.
\end{abstract}
...
variables. Missing data must be indicated by missing data codes,
...
Missings in the covariates are replaced by the global mean.

In the current implementation of \si, missing data
in the dependent variables are treated in a simple but meaningful way,
trying to minimize their influence on the estimation results.
...
For bipartite networks the Huisman-Snijders method can be used
only for the actors (the first node set), not for the second node set.
For the maximum likelihood estimation option, the Huisman-Snijders method
is not implemented. In these cases only the structural zeros method can be used.
...
Changes in RSiena and RSienaTest:
  \begin{itemize}
   \item Correct bug in EffectFactory for \texttt{isolatePop} effect.
   \item Improved plotting of sienaGOF objects so that observed values
     outside of the range of simulated values don't run off the chart.
   \item Improve treatment of structural values in sienaGOF.
  \end{itemize}
Changes in RSiena:
  \begin{itemize}
   \item Add functions AntiIsolateEffect.h and AntiIsolateEffect.cpp
...
Changes in RSiena as well as RSienaTest:
  ...
  \item Improved printing of results of siena07() in the case simOnly.
  \item Prettier response printed to console for includeEffects() and setEffect().
