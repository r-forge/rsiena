...
This version was renewed for \RS version 1.1-282.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-282) and an R-forge revision
...
\begin{itemize}
   \item Effects \texttt{cl.XWX} and \texttt{cl2.XWX} corrected (thanks to Christoph Stadtfeld)
     (\sfn{MixedTwoPathTable.cpp}).
   \item \texttt{interactionType} of \texttt{gwesp..} effects was made \texttt{dyadic} 
      (\sfn{allEffects.csv})
   \item Some layout changes in warning message, \sfn{phase3.r}.
   ...
   \item Components \texttt{IActors} and \texttt{expectedI} added to \sfn{sienaRI()}
     ...
     with the parameter \texttt{relative} in \sfn{plot.sienaRI()}.
