...
\nnm{sienaDataCreate}. (Details yet to be documented!)

Effect objects of class \nnm{sienaEffects} are created by the function
\nnm{getEffects}.  (Details yet to be documented!)
...
\STATE create a list of nodesets from the first dependent variable
\ENDIF
\FORALL{covariates}
...
\STATE set diagonal to missing so is ignored in mean and range
\ENDIF
\STATE Create attributes: (a class method) \\
\sfn{ }\nnm{mean} ignore missings\\
\sfn{ }\nnm{range} Extent of range, ignore missings. Make sure a double.\\
...
\ENDIF
\IF{both nodesets the same}
\STATE set all diagonals to missing so are ignored in mean and range
\ENDIF
\STATE Create attributes: (a class method) \\
\sfn{ }\nnm{mean} ignore missings\\
\sfn{ }\nnm{range} Extent of range, ignore missings. Make sure a double.\\
\sfn{ }\nnm{name} name of object
\IF{both nodesets the same}
\STATE set all diagonals to zero
\ENDIF
\end{algorithmic}
...
\label{sec:nodeset}
\begin{algorithmic}
\IF{nodeset name is ``Actors''}
\STATE Valid
\ELSIF {Nodeset name in the list and lengths match}
\STATE Valid
\ELSE
\STATE Invalid
\ENDIF
...
\sfn{ }\nnm{uponly} TRUE if all changes increase
\sfn{ }\nnm{downonly} TRUE if all changes decrease
...
\sfn{ }\nnm{range} Min and max of non-structural values\\
\ELSIF{bipartite}
\STATE Create attributes:\\
\sfn{ }\nnm{balMean} see section(\ref{sec:balmean})\\
\sfn{ }\nnm{symmetric} FALSE\\
...
The function \sfn{networkRateEffects} creates the required number of
rate effects for networks.  \sfn{createEffects} extracts the rows
from the effects data frame for an effect group and calls the function
\sfn{substituteNames} to replace the variable fields by the current variable
name.
...
rate effects
\IF {not poszvar}
\STATE reduce the new objective function effects to the rows 1 and 2 (symmetric)
or 3 (non symmetric)
\ENDIF
\IF{ not morethan2}
\STATE remove the second objective function effect
\ENDIF
\end{algorithmic}
...
function effects
\IF{first node set matches}
\STATE reduce the objective function effects to the third row
\ELSIF {not poszvar}
\STATE reduce the new objective function effects to the rows 1 and 2
\IF{ not morethan2}
\STATE remove the second objective function effect
\ENDIF
\ELSE
...
\item[dependent variables] balance mean, symmetric, missing, structural,
similarity mean, poszvar, range, min, max, moreThan2,  missing, structural
\item[changing covariates] similarity mean, mean, range, min, max, poszvar,
  moreThan2
\item[constant dyadic covariates] mean, min, max
\item[changing dyadic covariates] mean, range
...
\item[symmetric] logical vector indicating whether the corresponding independent
  variable is symmetric (set to FALSE for behavior variabnles and bipartite
  networks.
...
\item[periodNos] A list of the period numbers (misses out the final one for each
  data object)
\item[netnodesets] A list containing the nodeset(s) for each independent
  variable
...
\item[\nnm{x}] Input model object, as described under \nnm{sienaModelCreate}.
