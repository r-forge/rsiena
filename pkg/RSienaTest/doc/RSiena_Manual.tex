...
%\put(0,0){\includegraphics\item[scale=4]{ilcampo.jpg}}
%\end{picture}
\vfill
\begin{center}
...
This version was renewed for \RS version 1.1-301.
\end{abstract}
...
More specific references are \citet{Schweinberger12} for the score-type tests
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-301}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-301.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-301.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-301.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-301.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-301.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-301.tar.gz}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-301) and an R-Forge revision
...
The Generalized Method of Moments is somewhat more time-consuming
and is theoretically more efficient than the Method of Moments.
On the other hand, function \sfn{sienacpp()} is coded in a more efficient
way than \sfn{siena07()}.
...
In not-so-nice situations (very small network data sets,
small network and behavior data sets in combination with complex models),
however, ML and Bayesian methods tend to produce somewhat more accurate results
than MoM.
...
The list and order of the randomly varying effects, except for the rate parameters,
is shown by requesting
\begin{verbatim}
myeff[myeff$randomEffects & myeff$include, ]
\end{verbatim}
if the effects object is called \texttt{myeff}.
\smallskip
...
to be informative for final results. They may be disregarded.
\medskip
...
% from tikztry.tex
\\
\emph{Figure.} Geometrically weighted edgewise shared partners; \\
...
\emph{Figure.} GWESP weight for a tie closing $s$ two-paths for
...
\begin{enumerate}
\setcounter{enumi}{\value{savenumi}}
\item \emph{Simmelian outdegree effect}, (\texttt{simmelian}), the effect of
...
\item \emph{Simmelian alter X effect}, (\texttt{simmelianAltX}), the effect of
...
\end{enumerate}
\smallskip
The following six are degree-related effects, where nodal degrees
...
\texttt{inPopIntnX, inActIntnX, outPopIntnX, outActIntnX}, the $W$-degrees 
...
\texttt{inPopIntnX, inActIntnX, outPopIntnX, outActIntnX} with the ego or
alter effects of $V$.
...
\setcounter{enumi}{\value{savenumi}}
 ...
 \item {\em Effect of out-degree in W, weighted by sender's V, on X-popularity }
 ...
 \item {\em Effect of out-degree in W, weighted by sender's V, on X-activity }
...
\item \emph{average Simmelian alter effect}, (\texttt{avSimmelianAlt}), the effect of
...
\item \emph{total Simmelian alter effect}, (\texttt{totSimmelianAlt}), the effect of
...
\begin{followenum}
 ...
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ij}\,x_{2ij} \,$;
 \item {\em double indegree effect} \texttt{(BBDeg)},\\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ji}\,x_{2ji} \,$;
 \item {\em combined out-indegree effect} \texttt{(FBDeg)},\\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ij}\,x_{2ji} \,$;
...
\item Set the parameter \texttt{firstg} in \sfn{sienaAlgorithmCreate} to a
   ...
   use a smaller value (but less than 0.0001 probably makes no sense).
...
\item In combination with trying out a lower value for \texttt{firstg},
    you may try setting a higher value for  \texttt{doubleAveraging}
...
This only presents changes which affect the user, until
...
\begin{itemize}
   \item Warning if \sfn{includeEffects} is used with parameter 
     \texttt{initialValue} -- corrected.
   \item New effects: \texttt{sameXCycle4, homCovNetNet, contrastCovNetNet, 
     covNetNetIn, homCovNetNetIn, \\
     contrastCovNetNetIn, inPopIntnX, inActIntnX, outPopIntnX, \\
     outActIntnX}.
   ...
   \item \sfn{SienaRI}: Structural zeros and ones are excluded from the calculations;
     added option \texttt{getChangeStatistics};
