...
This version was renewed for \RS version 1.1-306.
\end{abstract}
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-306}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-306.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-306.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-306.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-306.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-306.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-306.tar.gz}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.1-306) and an R-Forge revision
...
It is also reported in the output and available -- e.g., for the first
...
\item Unilateral initiative and reciprocal confirmation,
      ...
      This offset is a fixed parameter, not estimated. It is determined
      by the value of \texttt{UniversalOffset} in function
      \sfn{sienaAlgorithmCreate}. If there are multiple symmetric dependent
      networks that require different offsets, this can be given as a
      named vector; see the help page for \sfn{sienaAlgorithmCreate}.
      Plausible values for this offset are in the range from 0 to 3,
      and should increase with $n$.
\iffalse
...
\small{
\url{http://www.stats.ox.ac.uk/~snijders/siena/changeForbiddenChanges.R}}.
...
in principle the same effects are used as above; you may
search for 'non-directed' to see the cases where the definition of
