...
This version was renewed for \RS version 1.2-26.
\end{abstract}
...
\item 2020-08-12 R-Forge Revision 349, package version 1.2-26.

Changes in RSiena and RSienaTest:
\begin{itemize}
   \item \texttt{tcltk} no longer a requirement; if not available,
        batch mode is followed for \sfn{siena07}.
   ...
   \item For \sfn{siena.table}, some of the \texttt{effectNames} changed to nice strings,
     so that \LaTeX\ can run without errors if \texttt{type='tex'}.
   ...
   \item For \sfn{sienaDataCreate}: more extensive error message, and \texttt{class(...)} replaced
     by \texttt{class(...)[1]}.
   \item multiplication factor added to \sfn{print.sienaAlgorithm} if \texttt{maxlike}.
