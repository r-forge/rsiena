...
This version was renewed for \RS version 1.2-22.
\end{abstract}
...
\url{http://cran.r-project.org/web/packages/RSiena/} .\\
            ...
            the root name of the file is \texttt{RSienaTest\_1.2-22}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2-22.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2-22.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.2-22.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-22.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-22.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-22.tar.gz}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.2-22) and an R-Forge revision
...
\begin{enumerate}
  \item \texttt{projname}: the `project name', the root name of the file
       ...
       and have the name \texttt{ppp.out}, if \texttt{projname~=~ppp}.
...
\citep[see][]{Koskinen04,KoskinenSnijders07,SchweinbergerSnijders07c,KoskinenSnijders19}.
...
In the latter case, it may be quite arbitrary which groups are
...
\item \emph{in-isolate Outdegree effect}, (\texttt{inIsDegree}), the
 ...
 to \texttt{outTrunc} for $c=1$, but with the opposite sign.
 ...
 This holds for the \texttt{outTrunc}($c=1$) effect but, in modified form,
 ...
 creates a link, if the previous outdegree of $i$ is less than $c$.
 For $c=1$, this means a change statistic of +1 for the creation of the
 ...
 this is equivalent to \texttt{outTrunc} for $c=1$,
 but with a sign reversal for the parameter;


 \item {\em out-degree more than $c$} (\texttt{outMore})
 where $c$ is some constant
 (internal effect parameter, see above), defined by \\
 $s^{\rm net}_{i\vit}(x) = \text{max}(x_{i+}\,-\, c, \, 0)$;\\
...
\item 2020-01-06 R-Forge Revision 345, package version 1.2-22.
...
\item Handling of mssings in changing covariates corrected (?).
\end{itemize}
...
\begin{itemize}
   ...
   \item \sfn{sienaBayes}: check that \texttt{prevAns} is not a \sfn{sienaFit} object;
     totally skip initial multigroup estimation if \texttt{prevAns} is given with
...
\begin{itemize}
   ...
   \item  Examples added to help page for \texttt{print.sienaBayesFit} (in dontrun...).
\end{itemize}
