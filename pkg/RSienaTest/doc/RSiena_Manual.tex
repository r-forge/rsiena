...
These means are stored as attributes on the object created by
\sfn{SienaDataCreate}.
...
results1 <- siena07(model1, data = mydate, effects = myeff,
      ...
      \texttt{results0} was the same as \texttt{myeff}, then also the initial
      values are copied, and Phase 1 of the algorithm is skipped,
...
Wald.RSiena <- function(A, ans)
{
    ...
    c(chisquare = chisq, df = d.f., pvalue = pval)
}
\end{verbatim}
...
in \SI \citep[see][]{Schweinberger11}.
...
Fixed some bugs in ML estimation procedure which will alter the results slightly.
