...
edges <- cbind(tmp@i + 1, tmp@j + 1, tmp@x)
\end{verbatim}
...
\item If \texttt{useStdInits = FALSE} and no \textsf{prevAns}
      ...
      \hyperlink{T_change_init}{Below, we give a way to change the values} contained in
      the \textsf{sienaEffects} object.
\item If \texttt{useStdInits = FALSE} and the \textsf{prevAns} (`previous answer')
...
results1 <- siena07(model1, data = mydate, effects = myeff,
      ...
      \textsf{sienaFit} object; in this example it is given as \texttt{results0}.
\end{enumerate}
...
\noindent
\hypertarget{T_change_init}{\emph{Changing the starting values in the effects object}}
\smallskip
...
\begin{tabbing}
 ...
 \texttt{ans\$msf   }                 \>  covariance matrix of  \texttt{ans\$sf    } \\
 \texttt{ans\$dfra  }                 \> estimated derivative of expected statistics w.r.t.\ parameters  \\
 \texttt{ans\$effects }               \> effects object  \\
\end{tabbing}
Like for any \R object, the internal structure of the effects object
...
is used (the default \texttt{batch = FALSE} in the call of \textsf{siena07}),
...
is called \texttt{ans}, it was mentioned above
that requesting just \texttt{ans} or \texttt{summary(ans)}
produce output on the \R console.
A table in latex or htlm format can be produced from this output by the
\textsf{xtable.sienaFit} method.
For example.
\begin{verbatim}
xtable(ans, file="ans1.htm", type="html")
\end{verbatim}
produces in the working directory a html file with the \texttt{ans} results in
tabular form. The \textsf{xtable} library has many further options.

The function \textsf{siena07}
...
The output file is divided into sections indicated by a line {\tt @1},
...
For conditional estimation, the rate parameters are
...
\subsubsection{Meta-analysis directed at the mean and variance of the parameters}
...
\item Test $H_0^{(1)}:\ \mu_\theta = 0$.\\
      This is done by means of a standard normal test statistic $ t_{\mu_\theta}$,
      being the ration of the estimate for $\mu_\theta $ to its standard error.
...
Cochran (1954) and Snijders and Baerveldt (2003).
The second is a maximum likelihood method under the assumption
of normal distributions; the associated confidence intervals
...
in that case, the number $N$ mentioned above
is the number of data sets actually used for the
parameter under consideration.

For these two approaches, it is assumed that the true deviations $\theta_j - \theta$
...
The maximum-likelihood based methods also assume that the true values
	$\theta_j$ are normally distributed in the population.
If this is a reasonable approach, the maximum-likelihood based methods
are preferable.
...
phenomenon in logistic regression.) Unfortunately, it is impossible to say in
...
\item for detecting if there are any networks with a positive parameter value,
       ...
       $H_0 :$   For all networks, the value of this parameter is zero or less than zero;\\
        ...
        $H_1 :$
             For at least one network, the value of this parameter is greater than zero;
...
\end{enumerate}
      ...
      (e.g., 0.025 if $\alpha = 0.05$) which yields an overall combined test at significance level $\alpha$.
      Note that four different overall results are possible.
      Indicating the right-sided and the left-sided p-values by $p_r$ and $p_l$,
...
\end{enumerate}
      ...
      i.e., under the combined null hypothesis that $\theta_j = 0$  for all $j$,
...
%       =  (n-2)b_0 - \big( x_{+i}+x_{+j} -x_{ij}-x_{ji}-2\sum_h x_{hi}x_{hj} \big)
 ...
 \item {\em out-degree up to $c$} (\emph{truncated out-degree}), where $c$ is some constant
...
\begin{enumerate}
 ...
 \item {\em quadratic shape effect, or effect of the behavior upon itself} \texttt{(quad)},
...
% where again $I\{A\}$ denotes the indicator function of the condition $A$;
 ...
 defined by the sum of centered similarity scores ${\rm sim}^z_{ij}$ between $i$
...
\begin{equation}
   u^{\rm beh} \,=\, \beta_{\rm trend} \, (z_i - \bar z)
...
\[
   u^{\rm beh} \,=\, 0.3618 \, (z_i - \bar z)
              \,-\, 0.06 \, (z_i - \bar z)^2 \,+\,
                   3.9689 \,  ({\rm sim}^z_{ij} - \widehat{{\rm sim}^z}) \ .
\]
...
Effects can be added to your personalized version of \rs.
...
of the package.
This section gives the outline of the procedure, and then presents
...
\item Open the folder
      \textsf{"RSiena\textbackslash src\textbackslash
		...
		\begin{itemize}
            ...
            \item In the file EffectFactory.cpp, at the appropriate place,
                  add the lines
                \begin{verbatim}
...
\item The name \sfn{OutTruncEffect} was chosen for the new function.\\
...
\end{verbatim}
      were inserted.
      This refers the program, when it encounters short name \sfn{outTrunc},
      to the function \sfn{OutTruncEffect}. The next step was to construct
      this function.
\item To choose a template for \sfn{OutTruncEffect}, we could make various
