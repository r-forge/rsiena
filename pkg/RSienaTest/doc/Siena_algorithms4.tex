...
is defined by the sum over the waves from 2 to $M$ of
\begin{equation}
 s_{k}^X(x,z) = \sum_i s_{ik}^X(x,z) \ .        \label{fitstat}
\end{equation}
If the number of  dependent variables is more than one, like in studies
of networks and behavior, for wave $m \in \{2, \ldots, M\}$, 
the dependent variable is taken as in wave $m$ and the 
explanatory as in wave $m-1$. This is explained for 
the case of networks and behavior in \citet{SnijdersEA07}.

In the C++ code, (\ref{fitstat}) is the function \nnm{evaluationStatistic()}.
The terms in (\ref{fitstat}) are the function \nnm{egoStatistic()},
...
\begin{equation}
 ...
 \hspace{3em}   \frac{T^w_r}{\rho^w_r} - \sum_{\text{active } i}
