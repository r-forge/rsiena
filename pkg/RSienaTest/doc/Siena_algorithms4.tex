...
\newcommand{\addcf}[1]{{\textcolor[named]{Red}{#1  (changed 30/05/2010)}}}
...
\remark{The additions in the version of May 30, 2010, pertain
to the following.
\begin{enumerate}
...
\end{enumerate}
Again, all changes are indicated in red.
}
...
(using $m=1$).
\begin{itemize}
\item All variables unobserved variables are prior independent.
...
\item  $y \leftarrow y_{\text{init}}$ ;\\[1em]
      ...
      \textit{StepProb2}$(y,w_0,i_0,j_0,r_0,-d_0;  rr_0, lospr_0, lcpr_0)$;\\[1em]
        \textit{sumlprob\_new} $ \leftarrow \textit{sumlprob\_new} + lcpr_0 + lospr_0 $ ;\\[1em]
      If (not \textit{SimpleRates}), then
      \begin{enumerate}
      \item \textit{mu\_new} $ \leftarrow  \textit{mu\_new} + rr_0 $ ,
      \item \textit{sigma2\_new} $ \leftarrow
...
\item If \textit{SimpleRates}, then
      \begin{equation}
        \textit{KappaFactor} \leftarrow  rr \ttimes ( \textit{TotNumber }-1)  \nonumber
        \end{equation}
      else (i.e., if not \textit{SimpleRates})
      \begin{equation}
      \textit{KappaFactor} \leftarrow \sqrt{ \frac{\textit{sigma2}}{\textit{sigma2\_new}} }
...
\begin{enumerate}
\item At initialization, a burn-in is necessary (only once, at the start of the
...
\begin{equation}
 \lambda^w(\theta, r,i, y) = \rho^w_r \, \exp(\alpha^w_{r} s^w_{ri} )
               \label{tkappa5}
\end{equation}
(where $\alpha^w_{r}$ is a row vector and $ s^w_{ri}$ a column vector).
...
If there is composition change, denote the time duration
for which actor $i$ is present in a given period between two waves
by $(td)_i$.
Note that total time duration between two waves
always is 1.
Thus, $0 \leq (td)_i \leq 1$, and $(td)_i = 0$ for actors
who are absent throughout this period, $(td)_i = 1$ for actors who
always are present, and $0 < (td)_i < 1$ for actors who join
or leave during the period.
\vspace{\parskip} \\
\hspace{\parindent}  % Just a \par does not agree with addce.
...
\frac{\partial\, \text{complete data log-likelihood}}{\partial \rho^w_r} =
    \frac{T^w_r}{\rho^w_r} - \sum_i (td)_i \, \exp(\alpha^w_{r} s^w_{ri}) \  , \label{tscore2}
\end{equation}
...
\begin{eqnarray}
  ...
  &=&                                                         \label{tscore4}
   \sum_i  s^w_{rih} \big(T^w_{ri} - (td)_i \, \rho^w_r \,
                   \exp(\alpha^w_{r} s^w_{ri} )  \big) \\
   &=& \sum_i  s^w_{rih} \big(T^w_{ri} - (td)_i \,  \lambda^w(\theta, r,i, y)   \big)
               \ . \nonumber
\end{eqnarray}
...
\[
   \lambda^+(\theta, +,+, y) = \sum_{r=1}^{R_N} \sum_i \lambda^N(\theta,r,i,y)
           +  \sum_{r=1}^{R_B} \sum_i \lambda^B(\theta,r,i,y)
\]
...
\addce{
In the Robbins-Monro algorithm we might use an adaptation,
and solve right away for $\rho$ using
the score function for $\rho$  --
consider (\ref{tscore2}), (\ref{tscore3}), and (\ref{tscore5}) --
given that the parameters $\alpha$ have been updated earlier.
(I hope this agrees with the stochasticity of the Robbins-Monro algorithm.)
}
...
\begin{equation}
 \lambda^w(\theta, r,i, y) = \rho^w_r \, \exp(\alpha^w_{r} s_{ri} )
               \label{kappa5}
\end{equation}
(where $\alpha^w_{r}$ is a row vector and $ s_{ri}$ a column vector).
