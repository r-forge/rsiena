...
\date{November 19, 2010, 2nd version }
...
\newpage
      ...
      \end{enumerate}
\item If \textit{misdat}, let $\textit{ThisLength} \leftarrow c$.\\
      \remark{This is new in the version of November 17, 2010. The earlier version
      (where \textit{ThisLength} could remain being larger than $c$) was not wrong,
      but carried out some superfluous mutually canceling subtractions and additions.}
\item Let \textit{perm} be a random permutation of the numbers $1$ to $c$.
\item \addce{
...
\item For $s$ running from 1 to $c$, do:
      ...
      \end{enumerate}
\item if (not \textit{misdat}) \remark{(this condition was added in version 17 Nov 2010
      to avoid superfluous additions and subtractions)} :\\
...
%            }  unnecessary; then (\ms_a, \ms_b) would not be a CCP.
      ...
      \item if \textit{misdat}:
            ...
            \remark{This condition was added in the version of November 17, 2010,
                    ...
                    in step 1.\ of procedure \textit{MH\_DelPermute}.}
            \end{enumerate}
      \end{enumerate}
\vspace*{-1em}
...
\item For $1 \leq s \leq \textit{ThisLength}-2$ \
      denote by $\textit{Coordinates}_s$ the values
      $(w,i,j,r,d)$ of $\ms_a.\nnm{succ}^{s}$;\\
      if \textit{misdat}, this is required only for $1 \leq s \leq c$.
      \remark{Last remark new in version 17 Nov 2010.}
\item Note that still $y =  \textit{StateBefore}(\ms_a)$ as was assigned above.\\
      For $s$ running from 1 to $c$, do:
      \begin{enumerate}
      \item \textit{StepProb2}$(y, \textit{Coordinates}_{\textit{perm}(s)};
      ...
      \end{enumerate}
\newpage
\item If not \textit{misdat} \remark{(this condition added
      version 17 Nov 2010)} then for $s$ running from $c+1$ to $\textit{ThisLength} -2$ do:
      \begin{enumerate}
...
\remark{This section is changed in the version of November 19, 2010.}
...
\lambda_{++} = \sum_{i,j} \lambda_{ij} =
       \sum_{\text{active }i} \ \sum_{\text{active }j, j \neq i} \lambda_{0i}\lambda_{0j}   \label{eq: lambda++}
\end{equation}
...
\remark{if the functions $\lambda_{0i}$
