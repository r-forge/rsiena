...
\usepackage[pdfstartview={},pdftex]{hyperref}  % ,colorlinks
...
\remark{Compilation of this Latex file leads to warning messages.
       These disappear when hyperref is not loaded.
       I suppose they are due to a bug in hyperref.}
...
$g^N(r,i,j,x,z)$ \hspace*{3em} \> endowment function function, network $r$\\
                     \> (used also as creation function!) \\[1ex]
...
\item \label{itemx}
      ...
      For all $j \in C$ with $N^{r}_{ij} = 0$, calculate $h_j = h_j + g^N(r,i,j,x,z)$.\\
...
For an effect $s_{ik}^X(x,z)$, the change statistic or change contribution is defined by
\begin{equation}
...
The statistic used for estimation is defined by
\begin{equation}
 s_{k}^X(x,z) = \sum_i s_{ik}^X(x,z) \ .        \label{fitstat}
\end{equation}
...
\begin{enumerate}[resume]
\item \addcf{This item is used only if not \textit{misdat} :\\
      ...
      \[
             \textit{NewCCPNumber} \leftarrow \textit{CCPNumber} + I\{d_- \times d_0 = -1\}
                        +  I\{d_+ \times d_0 = +1 \}
      \]
...
\addce{
      \item If $\textit{ThisLength}-2 \leq c_0$, \\
            then $\ms_g \leftarrow  \ms_b.\nnm{pred}$,\\
             else $\ms_g \leftarrow \ms_a.\nnm{succ}^{(c+1)}$ .
...
\item  $ (w_0,i_0,j_0,r_0)  \leftarrow  \emph{RandomInitMis}$ ; \\
      if $ w_0 = B$, then
        ...
        \end{enumerate}
\item   If $w_0 = B$, and adding $d_0$ to
...
\frac{\partial\, \text{complete data log-likelihood}}{\partial \rho^w_r} =
    \frac{T^w_r}{\rho^w_r} - \sum_{\text{active } i}  \exp(\alpha^w_{r} s^w_{ri}) \  , \label{tscore2}
\end{equation}
...
\\
\hspace*{\parindent}
The score functions now are given
for the basic rate parameters by
\begin{equation}
\frac{\partial\, \text{complete data log-likelihood}}{\partial \rho^w_r} =
    \frac{T^w_r}{\rho^w_r} -
...
but it will be potentially a smaller set if one of the conditions uponly, downonly,
higher, disjoint, or atleastone holds.
...
The probability of change is
      \begin{align}
       ...
       &= p_{1ij}(x, \beta) \, \big( p_{0ji}(x, \beta)  \big)^{(1-x_{ij})}
         =   p_{1ij}(x, \beta)\, \big(x_{ij} + (1-x_{ij})p_{0ji}(x, \beta) \big)\ ,   \label{eq:cM}
      \end{align}
...
$\alpha_k$ (including $\rho$) is given by
\begin{equation}
    \frac{\partial }{\partial \alpha_k} \log \Big( \lambda_i e^{-t\,\lambda_+} \Big)
  = \frac{1}{\lambda_i} \frac{\partial \lambda_i}{\partial \alpha_k} \, - \,
  t  \frac{\partial \lambda_+}{\partial \alpha_k} \ .
...
The term $\sum_{j} z_{jk}\lambda_j $ should be calculated only once.
...
For the two-sided opportunity (D.2, M.1 and C) the rate of change is
the product of two actor-dependent rates,
\begin{equation}
\lambda_{ij} = \lambda_{0i}\lambda_{0j} = \rho^2\exp\Big(\sum_k \alpha_k (z_{ik} + z_{jk})\Big) \ .\label{eq:lambda2}
\end{equation}
(This may be extended later - this is how it was implemented in Siena 3.)
A ministep by the pair of actors $i$ and $j$,
when associated with an elapsed waiting time $t$, contributes
to the probability density of the entire process a factor
\[
\lambda_{ij} e^{-t\,\lambda_+}
\]
where $\lambda_+ = \sum_{i,j} \lambda_{ij}$ is the total rate of change,
the summation being over all pairs of active actors with $i \neq j$.


[If conditions such as uponly are implemented, an extra decision must be made
for how to exclude the non-permitted cases: first loop over $i$; in the inner loop over $j$,
do we use `choose $j$; if not permitted, exit from inner loop', or `choose $j$ until
a permitted $j$ is found'? (In the second case, $i$'s without any permitted $j$'s
must not be active). This affects the definition of $\lambda_+$. The raw sum given above
corresponds to the first option.]

The development further is totally analogous to the case of one-sided opportunity.
The score function with respect to any parameter
$\alpha_k$ (including $\rho$) is given by
\[
    \frac{\partial }{\partial \alpha_k} \log \Big( \lambda_{ij} e^{-t\,\lambda_+} \Big)
  = \frac{1}{\lambda_{ij}} \frac{\partial \lambda_{ij}}{\partial \alpha_k} \, - \,
  t  \frac{\partial \lambda_+}{\partial \alpha_k} \ .
\]
This yields the following contributions to the overall score function.
...
\end{equation}
where the $\sum_{g,h}$ is taken over the set of all active actors, and what was said above
between square brackets applies again.


Certainly the term
$\sum_{g,h} (z_{gk}+z_{hk})\lambda_{gh} $
should be calculated only once.
In regular cases, where there are no uponly etc.\ conditions, this will not require
the execution of a double summation.
...
\addce{This appendix is changed. }
%\subsection{The factor $\kappa$}
