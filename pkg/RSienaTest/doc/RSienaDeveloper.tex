...
\end{verbatim}
But remove the carriage return.  The tests tend to fail for no reason,
particularly on laptops. Usually in ML runs, so there may be some bug there, but
it is hard to find as it is intermittent. It is wise to run them on a desktop or
server and
