...
\item Documentation is provided via Doxygen: install this and Graphviz. Use the
  doxygen wizard: File/open \texttt{config.dox} (in \texttt{RSienaTest/doc}).
  On a Mac, you need to use the expert/dot tab in the Wizard, and enter
  \texttt{/usr/local/bin} in the box marked DOT\_PATH.
...
\item Use upper case forthe preprocessor comments at the beginning and end of
  the header files.
...
These notes now apply to using R 2.12.0 or later. Please ensure you have the
...
\item You now should have a new version of the package ready for use. (If on 64
  bit Windows or Mac, you will have two versions.)
...
\item On linux or Mac, the same commands should work, but you don't need the
  ...
  use, within R, \sfn{install.packages("tarball name", repos=NULL)}. Or just
  open up R, and do \verb|install.packages('xtable')| which should create a
  local library directory for you.
\item If you find a Mac keeps recompiling all the C, even when you have not
  changed anything, comment out, using \#,
 the line beginning \verb|$(OBJECTS)| in the file
  \verb|RSiena/src/Makevars|. But try to remember not to commit this version!
\end{enumerate}
...
\item Please set environment variables to ensure you have tested everything you
  can. Details in section \ref{below}. You should have no warnings at all from
...
some suggested packages which are not actually used in the code aren ot
installed.
\item[build.Renviron] This is the file used for the build procedure. I suggest
\begin{verbatim}
_R_BUILD_RESAVE_DATA=no
\end{verbatim}
  On Windows you could also set \verb|CYGWIN=nodosfilewarning| here (and for
  portability it will not matter on Linux or Mac).
\end{description}
