...
\item Avoid unnecessary \sfn{for} loops. They are acceptable where a complicated
  ...
  \item Check that {R CMD check} works with no warnings apart from line endings
    (these are set to follow the platform, and will be OK when checked out on a
    Linux box).
...
\item On linux or Mac, the same commands should work, but you don't need the
  ...
  can't remember which one worked: one always did.
\end{enumerate}
...
need the header file \sfn{"R.h"} which will probably already be included in any
file I have debugged!
