...
This version was renewed for \RS version 1.1-284.
\end{abstract}
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-282}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-282.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-282.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-282.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-282.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-282.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-282.tar.gz}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-284) and an R-Forge revision
...
and the estimation must be repeated.
Usually the best way to do this is by employing the
argument \texttt{prevAns} in the call of \sfn{siena07()}.
Given that the earlier result was already called \texttt{result1},
this is done, e.g., by
\begin{verbatim}
results1  <-  siena07(algorithm1, data = mydata, effects = myeff,
                      prevAns = results1)
...
siena07ToConvergence <- function(alg, dat, eff, ans0=NULL, ...){
    ...
    repeat {
        ...
        if (maxt > 3) {break}    # divergence without much hope
...
\iffalse
The use of \texttt{ans\$effects\$requested} in the definition of \texttt{maxt}
...
using the \texttt{prevAns} parameter
...
\subsubsection{Pressing on}

If convergence is difficult and simplifications or step-by-step approaches
as suggested above are not sufficient or undesirable,
one may try to obtain convergence by playing with the
settings of the estimation algorithm.
For the Method of Moments,
the main parameter for this purpose is \texttt{diagonalize},
which is one of the arguments set in \sfn{sienaModelCreate}.
The default value is 1.0.
For the technical background, see \texttt{Siena\_Algorithms.pdf}
which can be downloaded from the \SI website.
This parameter may range from 0 to 1; the value 1 gives
greatest stability; smaller values may give greater efficiency;
small values (0.5 or less) usually lead to a very unstable algorithm.
Advice for the use of this parameter is as follows.

\begin{enumerate}
\item Start with \texttt{diagonalize} = 1 (the default).
\item If an `on track' result has been obtained with \sfn{siena07()},
      use this for the \texttt{prevAns} parameter in \sfn{siena07()},
      with a smaller value for \texttt{diagonalize}; try the value
      0.6; if this leads to instability, try 0.7 or 0.8
\end{enumerate}

\noindent
The following example script illustrates this procedure.
...
siena07PressOn <- function(alg1, dat, eff, ans0, ...){
    ...
    repeat {
        ...
        cat(numr, maxt,"\n")     # report how far we are
        if (maxt < 0.10) {break} # success
        if (maxt > 2) {break}    # divergence without much hope
                           # of returning to good parameter values
        if (numr > 100) {break}  # now it has lasted too long
...
Or, when it is being estimated and the $t$-ratio for convergence is
...
If the model also contains some elementary effects, the objective
...
\[
 s^{\rm net}_{i\vit}(x) = \sum_j x_{ij} ({\rm sim}(\grave v)_{ij}
  - \widehat{{\rm sim}(\grave v)}) \ ,
\]
...
\]
 ...
 $\breve v_j^{(-i)}$ is as above in effect \texttt{(altDist2)}.\\
  In version 1.1-284, there still is some inconsistency in the centering by
  $\widehat{{\rm sim}(\breve v)}$ which can be the
 \emph{observed} mean of all similarity scores $\breve v_i$;
...
\item \emph{ego-(in-alter-distance-2) covariate - similarity} \texttt{(simEgoInDist2)},
      ...
      $j$ to whom $i$ has a tie,
\[
 s^{\rm net}_{i\vit}(x) = \sum_j x_{ij} ({\rm sim}(\acute v)_{ij}
  - \widehat{{\rm sim}(\acute v)}) \ ,
\]
...
\setcounter{enumi}{\value{savenumi}}
 ...
 $i \rightarrow j$ in the figure above. The effect is defined as\\
 $s^{\rm el}_{i\vit}(x) = x_{ij} \sum_{h; h \neq j}  x_{ih}\, w_{hj}\,$;\\
...
\item \emph{ego-(alter-W-distance-2) covariate - similarity} \texttt{(simEgoDist2W)},
      ...
      $j$ to whom $i$ has a tie,
\[
 s^{\rm net}_{i\vit}(x) = \sum_j x_{ij} ({\rm sim}(\grave v^W)_{ij}
  - \widehat{{\rm sim}(\grave v^W)}) \ ,
\]
...
\]
 ...
 \emph{original} covariate $v$,
 $\breve v_j^{(-i)}$ is as above in effect \texttt{(altDist2)},
 and  $\widehat{{\rm sim}(\breve v)}$ is the
 \emph{observed} mean of all similarity scores $\breve v_i$;
 this observed mean is defined by calculating the $\breve v_i$ values
 for each of the observations $t_1$ to $t_{M-1}$, and averaging
 these
...
\item \emph{ego-(in-alter-W-distance-2) covariate - similarity} \texttt{(simEgoInDist2W)},
      ...
      $j$ to whom $i$ has a tie,
\[
 s^{\rm net}_{i\vit}(x) = \sum_j x_{ij} ({\rm sim}(\acute v^W)_{ij}
  - \widehat{{\rm sim}(\acute v^W)}) \ ,
\]
