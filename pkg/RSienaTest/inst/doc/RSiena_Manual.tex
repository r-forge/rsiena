...
This version was renewed for \RS version 1.1-290.
\end{abstract}
...
Ruth M. Ripley, Tom A.B. Snijders, Zs\'{o}fia Boda,
 Andr\'{a}s V\"{o}r\"{o}s, and Paulina Preciado, 2015.
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-290}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-290.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-290.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-290.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-290.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-290.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-290.tar.gz}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-290) and an R-Forge revision
...
while using the \texttt{prevAns} parameter to use previous estimates as starting
...
\begin{itemize}
   \item \sfn{sienacpp()} added (programmed by Felix Schoenenberger); this includes
     gmm estimation (Amati - Snijders).
   \item settings model added (Snijders - Preciado; programmed by Mark Ortmann);
     totally experimental, not yet documented. 
   ...
   \item \sfn{glueBayes()}: add \texttt{p1} and \texttt{p2} to created object.
\end{itemize}
