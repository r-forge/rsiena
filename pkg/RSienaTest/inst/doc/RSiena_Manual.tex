...
This version was renewed for \RS version 1.1-289.
\end{abstract}
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-289}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-289.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-289.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-289.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-289.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-289.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-289.tar.gz}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-289) and an R-Forge revision
...
\begin{followenum}
 ...
 \item {\em double reciprocated -- outdegree effect} \texttt{(RFDeg)},\\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ij}\,x_{1ji}\,x_{2ij}\, $;
 \item {\em double reciprocated -- indegree effect} \texttt{(RBDeg)},\\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ij}\,x_{1ji}\,x_{2ji}\, $;
