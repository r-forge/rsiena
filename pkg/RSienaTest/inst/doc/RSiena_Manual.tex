...
\hyphenation{Snij-ders Duijn Huis-man Steg-lich Schwein-ber-ger siena-Data-Create-From-Session
             siena-Data-Create siena-Com-position-Change RSiena-Test}
...
This version was renewed for \RS version 1.1-279.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-279) and an R-forge revision
...
\emph{diffusion of innovations} \citep{Greenan14}:
here the behavior variable representing
having adopted the innovation is binary, coded 0 or 1, and
...
% Now implemented as generic network effect.
% Perhaps the change statistic is not correct.
...
\setcounter{enumi}{\value{savenumi}}
 ...
 This is a generic effect, not an evaluation effect,
 ...
 Although technically this is not an evaluation effect, its type
 in \RS still is \texttt{eval};
\item
 ...
 In other words, only the $i \rightarrow h$ tie in the figure
 is the dependent variable here.\\
 Although technically this is not an evaluation effect, its type
 in \RS still is \texttt{eval}.
...
\texttt{(sameXInPop)} defined by the
 number of incoming ties received by the others to whom $i$ is tied
...
\setcounter{enumi}{\value{savenumi}}
 ...
 only as a consequence of the
 $i \stackrel{X}{\rightarrow} h $,
 $j \stackrel{W}{\rightarrow} h $ two-in-stars.\\
 Although technically this is not an evaluation effect, its type
 in \RS still is \texttt{eval}.
 \item
\begin{minipage}[t]{.7\textwidth}
...
because of the definition of a generic effect.)
...
for covariates on the first and on the second mode.)
...
\item 2014-11-13  Revision 279.

Changes in RSiena and RSienaTest:
\begin{itemize}
   \item Effects \texttt{AltsAvAlt} renamed to \texttt{avXAlt}.
   ...
   \item \texttt{Siena\_algorithms4.tex} renamed \texttt{Siena\_algorithms.tex};
         this document now is made available also at the \SI website.
