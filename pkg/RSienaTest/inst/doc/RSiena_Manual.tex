...
This version was renewed for \RS version 1.1-285.
\end{abstract}
...
\begin{enumerate}
	...
	\item	Install the package \RS or \sfn{RSienaTest}, with dependencies.
            ...
            the root name of the file is \texttt{RSienaTest\_1.1-285}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-285.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.1-285.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.1-285.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-285.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-285.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.1-285.tar.gz}
...
\item[Check your version of \RS]
  ...
  identified by a version number (e.g.\ 1.1-285) and an R-Forge revision
...
siena07ToConvergence <- function(alg, dat, eff, ans0=NULL, ...){
    ...
    repeat {
        ...
        if (tmax > 3) {break}    # divergence without much hope
...
\begin{itemize}
   ...
   * \sfn{sienaAlgorithmCreate()} has new arguments \texttt{n2start},
     \texttt{truncation}, \texttt{doubleAveraging}, \texttt{standardizeVar};
     this leads to various changes in \texttt{phase2.r}.
   * Diagonalization corrected (matrix transpose) (\texttt{phase2.r}).
